...
const socket = io();
// Tesseract variables
const tesseract_path = document.querySelector("#tesseract_path");
const tesseract_arguments = document.querySelector("#tesseract_arguments");
const tesseract_language_1 = document.querySelector("#tesseract_language_1");
const tesseract_language_2 = document.querySelector("#tesseract_language_2");
// Page variables
...
// Textarea variables
const text_area = document.querySelector("#text_field");
// Image setting variables
// -> X zoom
const x_zoom_input = document.querySelector("#x_zoom_range");
const x_zoom_value = document.querySelector("#x_zoom_value");
var g_x_zoom_factor = Number(x_zoom_input.value) / 100;
// -> Y zoom
const y_zoom_input = document.querySelector("#y_zoom_range");
const y_zoom_value = document.querySelector("#y_zoom_value");
var g_y_zoom_factor = Number(y_zoom_input.value) / 100;
// -> Rotation
const rotation_input = document.querySelector("#rotation_range");
const rotation_value = document.querySelector("#rotation_value");
var g_rotation = Number(rotation_input.value);
// -> DPI
const dpi_input = document.querySelector("#dpi_range");
const dpi_value = document.querySelector("#dpi_value");
var g_dpi = Number(dpi_input.value);
// -> Black & white binarization activation
const binarization_is_active = document.querySelector("#binarization_is_active");
var g_is_binarized = binarization_is_active.checked;
// -> Black & white binarization threshold
const binarization_input = document.querySelector("#binarization_range");
const binarization_value = document.querySelector("#binarization_value");
var g_binarization_threshold = Number(binarization_input.value);
// Canvas logic variables
// -> The canvas itself
const g_canvas = document.getElementById('viewport');
...
function handle_changed_text() {
    socket.emit("changed_text", text_area.value);
}
...
function handle_change_tesseract_arguments() {
    socket.emit("change_tesseract_arguments", tesseract_arguments.value);
}
...
function draw_rect(rect, rect_counter) {
    g_context.fillText(rect_counter.toString() + "_" + rect.language_state, rect.x * g_x_zoom_factor, rect.y * g_y_zoom_factor);
    g_context.strokeRect(rect.x * g_x_zoom_factor, rect.y * g_y_zoom_factor, rect.w * g_x_zoom_factor, rect.h * g_y_zoom_factor);
}
